...
# Install dependencies + GPG tools for MS Edge
RUN apt install -qqy \
        ca-certificates \
        curl \
        gnupg \
...
RUN echo "deb [arch=amd64 signed-by=/etc/apt/keyrings/microsoft.gpg] https://packages.microsoft.com/repos/edge stable main" > /etc/apt/sources.list.d/microsoft-edge-stable.list

# Install MS Edge
RUN apt update && \
    apt install -qqy microsoft-edge-stable
